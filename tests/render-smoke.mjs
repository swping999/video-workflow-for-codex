...
assert.equal(rendered.coverCandidates.length, 9);
...
for (const report of ["aesthetic-report.json", "cover-report.json"]) assert.ok(fs.statSync(path.join(project, "deliverables", report)).size > 100);
assert.ok(fs.readdirSync(path.join(project, ".media", "cache", "frames", "portrait"), { recursive: true }).some((entry) => String(entry).endsWith(".webp")));
