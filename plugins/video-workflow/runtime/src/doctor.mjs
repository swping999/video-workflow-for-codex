...
export function doctor({ requireSystemTts = true } = {}) {
  const failures = [];
  const nodeMajor = Number.parseInt(process.versions.node.split(".")[0], 10);
  if (nodeMajor < 22) failures.push(`Node.js 22+ is required; found ${process.versions.node}`);
  ...
  if (requireSystemTts && !systemProviderAvailable()) failures.push("Free system TTS is required for the default one-command build; install espeak-ng on Linux or use a local narration adapter / staged provider=files path");
  if (failures.length) throw new Error(`Environment check failed:\n- ${failures.join("\n- ")}`);
  return { node: process.versions.node, runtimeRoot, browser, systemTts: systemProviderAvailable() };
}
